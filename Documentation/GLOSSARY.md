...
| **Embedding** | A list of numbers (a vector, 384 numbers for bge-small) that encodes the *meaning* of a piece of text so that texts with similar meanings produce similar vectors. Computed by the embedding model and stored in `kb_chunks_vec`. Used to find semantically similar chunks even when they share no keywords with the query. | "vector", "dense representation", "encoding" |
| **ONNX (Open Neural Network Exchange)** | An open file format for storing a trained machine-learning model so it can run on different runtimes without needing the original training framework (e.g. PyTorch). Nexus uses ONNX Runtime (`onnxruntime`) to run the bge embedding and reranker models on CPU — this is ~5× lighter than importing PyTorch directly. | "model format", "serialized model" |
| **BAAI (Beijing Academy of Artificial Intelligence)** | The Chinese AI research lab that created and published the `bge` family of embedding and reranker models used by Nexus for local hybrid retrieval. Their models are hosted on HuggingFace under `BAAI/bge-*`. | "bge authors", "model provider" |
