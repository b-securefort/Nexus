...
This folder contains reference artifacts for the `drawio-diagrammer` skill and any other skill that generates `.drawio` files.

## Files

- `azureicons_drawio.txt`
  ...
  - Agent usage: call `read_kb_file` with path `kb/drawio/azureicons_drawio.txt` and scan for the service keyword (e.g. "Front_Door", "App_Service", "Cosmos").
...
- `awsicons_drawio.txt`
  ...
  - Agent usage: call `read_kb_file` with path `kb/drawio/awsicons_drawio.txt` and scan for the service keyword (e.g. "ec2", "lambda", "s3").
