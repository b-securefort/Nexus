...
- **`search_kb_hybrid`** — Preferred for KB content questions. Chunk-level hybrid search (BM25 + dense vectors, local, one embedding API call). Returns precise snippets with `source_url` citations.
- **`search_kb` / `read_kb_file`** — Use `search_kb` when hybrid index is warming. Use `read_kb_file` for full file context beyond a snippet. Fall back to `search_kb_semantic` only when hybrid returns nothing relevant.
