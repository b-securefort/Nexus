...
- **Workload tiers live IN the spine** — in a workload/flow diagram the app tier and data tier are spine stages, and when the workload's home is a spoke/VNet zone they are tiers INSIDE that zone box, reading with the flow: `gateway subnet | app tier | private endpoints | data` (the approved reference shape — see the `hub-spoke-workload` archetype). Do NOT scatter PaaS services as separate top-level planes "because PaaS isn't in the VNet" — that sprawls the canvas and forces align_to compensation; the zone box expresses *workload grouping*, not network containment. Show **App Service VNet integration** as a `private` edge from the app to a delegated integration `subnet`, never by duplicating the app into the subnets. PE for a PaaS → node in the consuming PE column with `private` edges; Managed Identity / Entra → identity stage at the head; Monitoring → its own `monitoring` stage at the tail, never inside a VNet; VM / VMSS / AKS / Bastion / AppGW / Firewall → inside the appropriate `subnet`.
- **align_to is a last resort** — at most one or two satellites, NEVER chained (A→B→C) and never a substitute for correct stage order; the engine reverts any align shift that would land a box on another box, so an abused align silently does nothing.
